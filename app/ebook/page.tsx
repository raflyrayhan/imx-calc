...
function toPreviewUrl(pdfUrl: string | null): string | null {
  if (!pdfUrl) return null;
  try {
  
    const u = new URL(pdfUrl, typeof window !== 'undefined' ? window.location.origin : 'http://localhost');
...
export default function EbookPage() {
  ...
  const reload = useCallback(
    async (opts?: { category?: Category }) => {
      ...
      try {
        ...
        const json = (await res.json()) as EbookRow[];
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((b) => {
      ...
      const matchesCat =
        !activeCategory || (b.category || "EBOOK") === activeCategory;
  ...
  const handleDelete = async (id: string, title: string) => {
    if (
      !confirm(
        `Delete "${title}" permanently? This will remove the PDF and cover.`
      )
    )
      return;
    try {
  ...
  return (
    <main className="relative min-h-screen">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 -z-10 bg-[linear-gradient(to_right,rgba(15,23,42,0.03)_1px,transparent_1px),linear-gradient(to_bottom,rgba(15,23,42,0.03)_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,rgba(255,255,255,0.04)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.04)_1px,transparent_1px)] bg-[size:24px_24px]"
      />
      ...
      >
        ...
        >
          E-Book{" "}
          <span className="text-indigo-700 dark:text-indigo-600 font-extrabold">
            Library
          </span>
        </motion.h1>
        <motion.p
        ...
        >
          Collections of e-books, articles, and resources for learning journey.
        </motion.p>
        <motion.div
          variants={fadeUp}
        ...
        >
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold shadow-sm border border-slate-300 bg-white text-slate-800 hover:bg-slate-50"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold border border-slate-300 bg-white text-slate-800 hover:bg-slate-50 lg:hidden"
          >
        ...
        <section className="mx-auto w-full max-w-6xl px-4 pb-6">
          ...
          >
            <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <div>
                ...
                >
                  ...
                  <option value="PRIVATE">Private (via API proxy)</option>
                  ...
                  {coverPreview && (
                    <div className="mt-3">
                      <img
                        src={coverPreview}
                        alt="cover preview"
                        className="max-h-32 w-auto rounded-md border border-slate-300 object-contain"
                      />
                    </div>
                  )}
                </div>
            ...
            <div className="mt-5 flex items-center justify-end gap-3">
              ...
              <button
                type="submit"
                className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold shadow-sm bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-60"
                disabled={isSubmitting}
              >
                <Upload className="h-4 w-4" />
            ...
            {filtered.map((b) => {
              ...
              return (
                <motion.div
                  key={b.id}
                  variants={fadeUp}
                  className="rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col h-full"
                >
                    ...
                    <div className="p-3">
                      {b.coverUrl ? (
                        <img
                          src={b.coverUrl}
                          alt={b.title}
                          className="max-h-32 w-auto object-contain rounded-md border border-gray-300 shadow-sm"
                          loading="lazy"
                        />
                      ) : (
                      ...
                      {b.pdfUrl ? (
                        <>
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-700 hover:bg-slate-50"
                          >
                            Open
                          </a>
                          <a
      ...
      {preview && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-3xl h-full bg-white shadow-2xl border-l border-slate-200 transform transition-transform duration-200 ease-out translate-x-0">
            ...
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur">
              <div className="min-w-0 pr-2">
                ...
                <p className="truncate text-xs text-slate-600">
                  {preview.author ?? 'Unknown'} {preview.year ? `• ${preview.year}` : ''}
                </p>
              </div>
              <button
                onClick={() => setPreview(null)}
            ...
            <div className="p-4 space-y-3">
              ...
              {toPreviewUrl(preview.pdfUrl)?.length ? (
                <div className="h-[70vh] w-full rounded-md border border-slate-200 overflow-hidden">
                  
                  <object data={toPreviewUrl(preview.pdfUrl)!} type="application/pdf" width="100%" height="100%">
                    <iframe src={toPreviewUrl(preview.pdfUrl)!} className="w-full h-full" />
                    <div className="p-4 text-sm text-slate-600">
                      Your browser cannot display the PDF.{' '}
                      <a className="text-indigo-600 underline" href={preview.pdfUrl!} target="_blank" rel="noreferrer">Open in new tab</a>.
                    </div>
                  </object>
                </div>
              ) : (
                <div className="rounded-md border border-slate-200 p-4 text-sm text-slate-600">
                  PDF is unavailable for preview.
                </div>
              )}
            </div>
          </div>
      ...
      <footer className="mt-10">
        <div className="mx-auto w-full max-w-6xl px-4 pt-8 pb-6 text-center text-sm text-slate-500 dark:text-slate-300">
          ...
          <strong className="text-indigo-700 dark:text-indigo-600">IMX</strong>{" "}
